...
impl fmt::Display for TestReport {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "========================================================")?;
        writeln!(f, "              LAR TEST RUNNER REPORT                    ")?;
        writeln!(f, "========================================================")?;
...
impl LarTestHarness {
    ...
    pub fn test_apk<P: AsRef<Path>>(apk_path: P) -> Result<TestReport, Box<dyn std::error::Error>> {
        ...
        report.record_pass(format!("Extracted {} ARM64 native libraries from APK", native_libs.len()));
        ...
        for lib in native_libs {
            ...
            match runtime.load_library(&lib.name, &lib.data) {
                Ok(loaded) => {
                    ...
                    report.record_pass(format!("{}: Mapped {} bytes", loaded.name, loaded.mem_region.len()));
    ...
    pub fn test_so_file<P: AsRef<Path>>(so_path: P) -> Result<TestReport, Box<dyn std::error::Error>> {
        let start = Instant::now();
        let mut report = TestReport::new();
        ...
        if is_16k_aligned(loaded.load_base) {
            report.record_pass(format!("{}: Load base 0x{:x} is 16KB aligned", loaded.name, loaded.load_base));
        } else {
            report.record_fail(format!("{}: Load base 0x{:x} is NOT 16KB aligned", loaded.name, loaded.load_base));
        }
    ...
    pub fn run_self_test() -> TestReport {
        ...
        let lib_game_data = generate_synthetic_arm64_so("libgame.so", "Java_com_example_renderFrame");
        ...
        report.record_pass(format!("Generated synthetic APK ({} bytes)", apk_data.len()));
        ...
        report.record_pass(format!("Extracted {} libraries from synthetic APK", extracted.len()));
        ...
        for lib in extracted {
            ...
            match runtime.load_library(&lib.name, &lib.data) {
                Ok(loaded) => {
                    ...
                    if is_16k_aligned(loaded.load_base) {
                        report.record_pass(format!("{}: 16KB alignment verified (base 0x{:x})", loaded.name, loaded.load_base));
                    } else {
                    ...
                    if lib.name == "libcore.so" {
                        if let Some(addr) = loaded.lookup_symbol("native_core_init") {
                            report.record_pass(format!("{}: Symbol 'native_core_init' resolved at 0x{:x}", loaded.name, addr));
                        } else {
                            report.record_fail(format!("{}: Symbol 'native_core_init' NOT found", loaded.name));
                        }
                    }

                    if lib.name == "libgame.so" {
                        if let Some(addr) = loaded.lookup_symbol("Java_com_example_renderFrame") {
                            report.record_pass(format!("{}: Symbol 'Java_com_example_renderFrame' resolved at 0x{:x}", loaded.name, addr));
                        } else {
                            report.record_fail(format!("{}: Symbol 'Java_com_example_renderFrame' NOT found", loaded.name));
                        }
                    }
                }
                Err(e) => {
        ...
        if qemu.is_available {
            ...
            report.record_pass("QEMU AArch64 not detected; LAR running in native/foreign host mode");
        }
